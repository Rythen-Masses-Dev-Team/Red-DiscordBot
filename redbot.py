...
try:
    ...
    print("RedBot needs Python 3.5 or superior.\n"
          "Consult the guide for your operating system "
          "and do ALL the steps in order.\n"
          "https://twentysix26.github.io/Red-Docs/\n")
    sys.exit()
...
#
#  RedBot, a Discord bot by Twentysix, based on discord.py and its command extension
#                   https://github.com/Twentysix26/
#
#
# RedBot.py and cogs/utils/checks.py both contain some modified functions originally made by Rapptz
#             https://github.com/Rapptz/RoboDanny/tree/async
#

description = "RedBot - A multifunction Discord bot by Twentysix"
...
def check_folders():
    folders = ("data", "data/redbot", "cogs", "cogs/utils")
...
def check_configs():
    if settings.bot_settings == settings.default_settings:
        print("RedBot - First run configuration\n")
        ...
        if "@" not in choice and len(choice) >= 50:  # Assuming token
            ...
            os.remove('data/redbot/settings.json')
            input("Invalid input. Restart RedBot and repeat the configuration "
                  "process.")
            exit(1)
    ...
    if not os.path.isfile("data/redbot/cogs.json"):
        print("Creating new cogs.json...")
        dataIO.save_json("data/redbot/cogs.json", {})
...
def set_logger():
    ...
    handler = logging.FileHandler(
        filename='data/redbot/discord.log', encoding='utf-8', mode='a')
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/redbot/red.log', encoding='utf-8', mode='a',
...
def set_cog(cog, value):
    data = dataIO.load_json("data/redbot/cogs.json")
    data[cog] = value
    dataIO.save_json("data/redbot/cogs.json", data)

def load_cogs():
    no_prompt = "--no-prompt" in sys.argv[1:]

    try:
        registry = dataIO.load_json("data/redbot/cogs.json")
    except:
        registry = {}
    ...
    if extensions:
        dataIO.save_json("data/redbot/cogs.json", registry)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if choice.strip() == "reset":
            shutil.copy('data/redbot/settings.json',
                        'data/redbot/settings-{}.bak'.format(int(time.time())))
            os.remove('data/redbot/settings.json')
